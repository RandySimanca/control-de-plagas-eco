...
export default function DocumentosLegales() {
  ...
  async function load() {
    ...
    try {
      ...
      const { data } = await api.get('/documentos-legales', { token })
      setDocumentos(data || [])
    } catch (err) {
      console.error(err)
  ...
  async function handleUpload(e) {
    ...
    try {
      ...
      if (!uploadRes.ok) {
        ...
        throw new Error(errData.message || errData.error || 'Error subiendo archivo')
      }
  ...
  async function handleDelete(doc) {
    ...
    try {
      const token = localStorage.getItem('token')
      await api.delete(`/documentos-legales/${doc.id}`, { token })

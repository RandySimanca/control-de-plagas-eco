...
export default function Configuracion() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (logo) {
        const ext = logo.name.includes('.') ? logo.name.slice(logo.name.lastIndexOf('.')) : ''
        ...
        if (!uploadRes.ok) {
          ...
          throw new Error(errData.message || errData.error || 'Error subiendo logo')
        }
      ...
      const token = localStorage.getItem('token')
      if (form.id) {
        const { data: updated } = await api.put(`/configuracion/${form.id}`, payload, { token })
        if (updated && updated.id) {
          setForm(prev => ({ ...prev, id: updated.id, logo_url: logoUrl }))
        }
      } else {
        const { data: created } = await api.post('/configuracion', payload, { token })
        if (created && created.id) {
          setForm(prev => ({ ...prev, id: created.id, logo_url: logoUrl }))
        }
      }

...
export default function Tecnicos() {
  ...
  async function load() {
    try {
      ...
      const [tecRes, ordenesRes] = await Promise.all([
        api.get('/profiles', { token, params: { rol: 'tecnico' } }),
        api.get('/ordenes-servicio', { token })
      ])
      const tecData = tecRes.data || []
      const ordenes = ordenesRes.data || []

      // Get order counts per technician
      const tecWithCounts = tecData.map((tec) => ({
        ...tec,
        ordenes_count: ordenes.filter(o => o.tecnico_id === tec.id).length
      }))

...
export default function OrdenDetalle() {
  ...
  // Recomendaciones y carga rapida de evidencias
  ...
  // Estados para mantenimiento de estaciones
  ...
  // Estado de edicion de areas
  const [showAreasModal, setShowAreasModal] = useState(false)
  const [selectedAreas, setSelectedAreas] = useState([])
  const [savingAreas, setSavingAreas] = useState(false)

  // Estado de edicion de metodos
  ...
  // Estado de edicion de actividad
  ...
  folio
        ...
        // Solo actualiza el folio si hace falta, normalmente ya existe
        await api.put(`/certificados/${id}`, { folio })
      }
  ...
  async function handleSaveEstaciones() {
    ...
    try {
      ...
      if (isOnline) {
        await api.delete('/estaciones-usadas', { params: { orden_id: id }, token: localStorage.getItem('token') })
        if (toInsert.length > 0) {
          await Promise.all(
            toInsert.map((row) => api.post('/estaciones-usadas', row, { token: localStorage.getItem('token') }))
          )
        }
        const data = await api.get('/estaciones-usadas', { params: { orden_id: id }, token: localStorage.getItem('token') })
        setEstaciones(data.data || [])
      } else {
        // Sin conexion: encolar borrados individuales con una operacion personalizada en metadatos
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Estaciones (interactivo para tecnico) */}
        <div className="card">
